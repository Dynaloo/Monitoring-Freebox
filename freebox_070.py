...
#
# version 0611
#         prise en compte des infos de telechargement
#
# version 0612 - juillet 2024 - by AuVeLiCo
#         Passage de v8 à v12 pour corriger un bug d'affichage des stations WiFi 
#               ENDPOINT="http://"+args.Endpoint+"/api/vXXX/" 
#
# version 070 - décembre 2024 - by AuVeLiCo
#         VPN Client info
#         VM info
#         Ajout statut SFP pour FTTH
...
def get_download(headers):
    api_url = '%s/downloads/' % ENDPOINT
    r = requests.get(api_url, headers=headers)
...
def get_download_stats(headers):
    api_url = '%s/downloads/stats/' % ENDPOINT
    r = requests.get(api_url, headers=headers)
...
# Ajout version 070
def get_vpnclient_status(headers):
...
def get_and_print_metrics(creds, s_switch, s_ports, s_sys, s_disk, s_lan, s_wifi, s_lan_interfaces, s_interfaces_hosts, s_static_dhcp, s_dynamic_dhcp, s_xdsl_tunnel, s_downloads, s_downloads_stats, s_status_vpn, s_status_vm):
...
# FTTH specific
    if connection_media == "ftth":
        ...
        if 'result' in json_raw:  
            ...
            my_data[tag1+"."+tag2+"."+tag3+"."+'sfp_pwr_tx'] = json_raw['result']['sfp_pwr_tx'] # Ajout version 070 / API UNSTABLE \
            my_data[tag1+"."+tag2+"."+tag3+"."+'sfp_pwr_rx'] = json_raw['result']['sfp_pwr_rx'] # Ajout version 070 / API UNSTABLE \
            my_data[tag1+"."+tag2+"."+tag3+"."+'sfp_link'] = json_raw['result']['link'] # Ajout version 070 / API UNSTABLE \
...
# -Y static dhcp    # Ajout 070 : n'affiche pas tout bien que 'i' s'incremente
    if s_static_dhcp:
...
# option -Q
    ...
    if s_downloads:
        json_raw = get_download(headers)    
        tag1="Telechargements"
        tag2="NULL"
        tag3="NULL"
        ...
        if json_raw['success'] :
            i=1
            if 'result' in json_raw:  
                for download_object in json_raw['result']:
                    tag2 = "dl-" + str(i)
                    if 'id' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'id'] = download_object['id']
                    if 'rx_bytes' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'rx_bytes'] = download_object['rx_bytes']
                    if 'tx_bytes' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'tx_bytes'] = download_object['tx_bytes']
                    if 'eta' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'eta'] = download_object['eta']
                    if 'status' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'status'] = download_object['status']
                    if 'io_priority' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'io_priority'] = download_object['io_priority']
                    if 'type' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'type'] = download_object['type']
                    if 'queue_pos' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'queue_pos'] = download_object['queue_pos']
                    if 'name' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'name'] = download_object['name']
                    if 'tx_rate' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'tx_rate'] = download_object['tx_rate']
                    if 'rx_rate' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'rx_rate'] = download_object['rx_rate']
                    if 'error' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'error'] = download_object['error']
                    if 'size' in download_object:my_data[tag1+"."+tag2+"."+tag3+"."+'size'] = download_object['size']
                    i=i+1

#
# option -T

    if s_downloads_stats:
        json_raw = get_download_stats(headers)
        tag1="Telechargements"
        tag2="Stats"
        tag3="NULL"
        ...
        if json_raw['success'] :       
            if 'result' in json_raw:  
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_stopped'] = json_raw['result']['nb_tasks_stopped']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_checking'] = json_raw['result']['nb_tasks_checking']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_queued'] = json_raw['result']['nb_tasks_queued']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_extracting'] = json_raw['result']['nb_tasks_extracting']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_done'] = json_raw['result']['nb_tasks_done']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_repairing'] = json_raw['result']['nb_tasks_repairing']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_active'] = json_raw['result']['nb_tasks_active']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_downloading'] = json_raw['result']['nb_tasks_downloading']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks'] = json_raw['result']['nb_tasks']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_error'] = json_raw['result']['nb_tasks_error']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_stopping'] = json_raw['result']['nb_tasks_stopping']
                    my_data[tag1+"."+tag2+"."+tag3+"."+'nb_tasks_seeding'] = json_raw['result']['nb_tasks_seeding']
...
#
    if s_status_vpn:
        ...
        vpn_json_raw = get_vpnclient_status(headers) 
        vpnstats = vpn_json_raw['result']
        if vpn_json_raw['result']['enabled']: 
            tag1="vpn"
            tag2="NULL"
            tag3="NULL"
            my_data[tag1+"."+tag2+"."+tag3+"."+'interface'] = vpn_json_raw['result']['active_vpn'] # Ajout version 070 / API UNSTABLE \ 
            my_data[tag1+"."+tag2+"."+tag3+"."+'last_up'] = vpn_json_raw['result']['last_up'] # Ajout version 070 / API UNSTABLE \ 
            my_data[tag1+"."+tag2+"."+tag3+"."+'type'] = vpn_json_raw['result']['type'] # Ajout version 070 / API UNSTABLE \ 
            my_data[tag1+"."+tag2+"."+tag3+"."+'name'] = vpn_json_raw['result']['active_vpn_description'] # Ajout version 070 / API UNSTABLE \     
            my_data[tag1+"."+tag2+"."+tag3+"."+'state'] = vpn_json_raw['result']['state'] # Ajout version 070 / API UNSTABLE \
            my_data[tag1+"."+tag2+"."+tag3+"."+'rate_up'] = vpn_json_raw['result']['stats']['rate_up'] # Ajout version 070 / API UNSTABLE \
            my_data[tag1+"."+tag2+"."+tag3+"."+'rate_down'] = vpn_json_raw['result']['stats']['rate_down'] # Ajout version 070 / API UNSTABLE \
            my_data[tag1+"."+tag2+"."+tag3+"."+'bytes_up'] = vpn_json_raw['result']['stats']['bytes_up'] # Ajout version 070 / API UNSTABLE \
            my_data[tag1+"."+tag2+"."+tag3+"."+'bytes_down'] = vpn_json_raw['result']['stats']['bytes_down'] # Ajout version 070 / API UNSTABLE \
            my_data[tag1+"."+tag2+"."+tag3+"."+'ip'] = vpn_json_raw['result']['ipv4']['ip_mask']['ip'] # Ajout version 070 / API UNSTABLE \
            my_data[tag1+"."+tag2+"."+tag3+"."+'gateway'] = vpn_json_raw['result']['ipv4']['gateway'] # Ajout version 070 / API UNSTABLE \
...
#
    if s_status_vm:
        vm_json_raw = get_vm_status(headers) 
        vmlist = vm_json_raw['result']
        l=len(vm_json_raw['result'])
        i=0
        while i<l:
            for vm in vmlist:
                tag1="vms"
                tag2=vm['mac']
                tag3="NULL"
                my_data[tag1+"."+tag2+"."+tag3+"."+'name'] = vm_json_raw['result'][i]['name'] # Ajout version 070 / API UNSTABLE \
                my_data[tag1+"."+tag2+"."+tag3+"."+'mac'] = vm_json_raw['result'][i]['mac'] # Ajout version 070 / API UNSTABLE \
                my_data[tag1+"."+tag2+"."+tag3+"."+'status'] = vm_json_raw['result'][i]['status'] # Ajout version 070 / API UNSTABLE \
                i=i+1
...
if __name__ == '__main__':
    ...
    parser.add_argument('-Q', '--status-downloads',
                        dest='status_downloads',
                        action='store_true',
                        help="Get and show downloads status")

    parser.add_argument('-T', '--status-downloads-stats',
                    dest='status_downloads_stats',
                    action='store_true',
                    help="Get and show downloads stats")
    ...
    parser.add_argument('-V', '--vpn_client',
                        dest='status_vpn',
                        action='store_true',
                        help="Get and show VPN Client status")
                        
    parser.add_argument('-M', '--vm_status',
                        dest='status_vm',
                        action='store_true',
                        help="Get and show VM status")
    ...
    ENDPOINT="http://"+args.Endpoint+"/api/v12/"
    auth = get_auth()
    ...
    if args.register:
        ...
        get_and_print_metrics(auth, args.status_switch, args.status_ports, args.status_sys, args.disk_usage, args.lan_config, args.wifi_usage, args.lan_interfaces, args.interfaces_hosts, args.static_dhcp, args.dynamic_dhcp, args.xdsl_tunnel, args.status_downloads, args.status_downloads_stats, args.status_vpn, args.status_vm)
